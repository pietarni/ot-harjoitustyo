...
class DirectionMap:
    ...
    # Goes through every elevation data pixel -
    # checks the neighboring pixels -
    # checks the relative elevation difference -
    # gets direction to that pixels, saves that as a vector.
    def interate_pixels(self):
        directionarr = np.full((len(self.inputarr[0]), len(
            self.inputarr), self.orientations, 3), 0.0)
    ...
    def calc_angle(self, neighbors, value):
        ...
        for x in range(-self.radius, self.radius+1):
            for y in range(-self.radius, self.radius+1):
                #We want to compare neighboring pixels to center pixels,
                #so ignore the center pixel.
                if not (x == 0 and y == 0):
                    if neighbors[x+self.radius][y+self.radius] >= 0:
                        ...
                        # add interpolated relative z distance of center pixel and neighbor to orientationsarr
                        orientationsarr[orientation] += (
                            neighbors[x+self.radius][y+self.radius]-value) / np.linalg.norm([x, y])
        ...
        for i in range(0, self.orientations):
            ...
            xyzdirections.append(
                (direction[0], direction[1], orientationsarr[i]))
